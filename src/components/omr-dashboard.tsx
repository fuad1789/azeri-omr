...
import {
  ...
  CLASS_CONFIGS,
  BURAXILIS_CONFIGS,
  DEFAULT_LEGACY_HEADER,
  DEFAULT_BURAXILIS_HEADER,
  HeaderField,
  parseOMRData,
  extractSubjectsFromLine,
} from "@/lib/omr-parser";
import { ParsingDebugger } from "./parsing-debugger";
import { StandardParsingDebugger } from "./standard-parsing-debugger";
            ...
            <div className="flex flex-col">
              ...
              <div className="relative">
                <input
                  ...
                  value={
                    subject.segments
                      ? `${subject.segments.reduce((acc, s) => acc + s.count, 0)} sual`
                      : subject.length
                  }
                  disabled={true}
                  readOnly
              ...
              <input
                ...
                value={
                  subject.points !== undefined
                    ? Number(subject.points).toFixed(0)
                    : 0
                }
                onChange={(e) =>
                  handleUpdateSubject(
        ...
        {isExpanded && subject.segments && (
          <div className="bg-slate-50 p-4 border-t border-slate-100 grid gap-3 animate-in slide-in-from-top-2 duration-200">
            ...
            <div className="text-center text-xs text-slate-400 mt-1 font-medium">
              ...
              Toplam Bal:{" "}
              {subject.segments
                .reduce((acc, s) => acc + s.count * s.points, 0)
                .toFixed(1)}
            </div>
          </div>
        )}
...
const SubjectKeyInputs = ({
  ...
  const getSubjectLength = (s: SubjectConfig) => {
    if (s.segments && s.segments.length > 0) {
      return s.segments.reduce(
        (acc, seg) => acc + seg.count * seg.lengthPerItem,
        0,
      );
    }
    return s.length || 0;
  ...
  const handleSegmentChange = (
    ...
    const currentSubjectValue = valFull
      .slice(subjectGlobalStart, subjectGlobalStart + subjectLength)
      .padEnd(subjectLength, " ");

    if (!subject.segments) {
      handleSubjectChange(
        subjectIndex,
        newValue,
        optLang === "ALL" ? undefined : optLang,
      );
      return;
    }
    ...
    subject.segments.forEach((seg, idx) => {
      ...
      const currentSegValue = currentSubjectValue.slice(
        segStart,
        segStart + segLen,
      );
    ...
    <div className="flex flex-col gap-3 w-full p-4 bg-slate-50/50 rounded-xl border border-slate-100 relative">
      ...
      <div className="flex flex-wrap items-start gap-6">
        {config.map((subject, subjectIdx) => {
          ...
          const isLanguageDependent =
            subject.id === "xariciDil" &&
            languages.length > 1 &&
            languages.some((l) => l !== "");
          const langsToRender = isLanguageDependent
            ? languages
            : [languages[0] || ""];
                    ...
                    >
                      ...
                      {!subject.segments ? (
                        <div
                          className="relative"
                          style={{
                            width: `${Math.max((subject.length || 0) + 4, 14)}ch`,
                          }}
                        >
                          <input
                            type="text"
                            ...
                            onChange={(e) =>
                              handleSubjectChange(
                                ...
                                isLanguageDependent ? lang : undefined,
                              )
                            }
                        ...
                        <div className="flex items-start gap-2">
                          {subject.segments.map((seg, segIdx) => {
                            ...
                            const segLen = seg.count * seg.lengthPerItem;
                            const segVal = rawSubjectValue.slice(
                              segStart,
                              segStart + segLen,
                            );
                            ...
                            if (seg.type === "open" || seg.type === "written") {
                              return (
                                <div
                                  key={segIdx}
                                  className="flex flex-col gap-1 w-full flex-1 min-w-[120px]"
                                >
                                  {showLabel && (
                                  ...
                                  <button
                                    onClick={() =>
                                      setEditingSegment({
                                        ...
                                        segmentIndex: segIdx,
                                        lang: isLanguageDependent
                                          ? lang
                                          : "ALL",
                                      })
                                    }
                                    className={cn(
                                      "w-full px-3 py-2 border border-slate-200 rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-2 h-[38px]",
                                      seg.type === "written"
                                        ? "text-amber-600 bg-amber-50 hover:bg-amber-100 hover:border-amber-300"
                                        : "text-indigo-600 bg-indigo-50 hover:bg-indigo-100 hover:border-indigo-300",
                                    )}
                                  >
            ...
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              {(() => {
                ...
                for (let k = 0; k < segmentIndex; k++) {
                  const s = subject.segments![k];
                  if (s && (s.type === "open" || s.type === "written"))
                    localOpenStart += s.count;
                }

                // Smart Validator Logic
                if (seg.type === "written") {
                  const currentWeights =
                    openWeights && openWeights[subject.id]
                      ? openWeights[subject.id]
                      : [];
                  const segmentWeights = currentWeights.slice(
                    localOpenStart,
                    localOpenStart + seg.count,
                  );
                  ...
                  const currentOpenTotal = Array.from({
                    length: seg.count,
                  }).reduce((acc, _, i) => {
                    return (
                      (acc as number) +
                      (segmentWeights[i] !== undefined
                        ? segmentWeights[i]
                        : seg.points)
                    );
                  }, 0) as number;
                  ...
                  return (
                    <div className="flex flex-col gap-1 w-full mr-8">
                      ...
                      <div className="flex items-center text-[10px] text-slate-500 gap-3">
                        <span>
                          Qapalı/Açıq: <b>{fixedScore}</b>
                        </span>
                        <span>
                          Yazı Limit: <b>{remainingForOpen}</b>
                        </span>
                        <span
                          className={
                            isOver
                ...
                return (
                  <h3 className="font-bold text-slate-700">Açıq Suallar</h3>
                );
              })()}
              <button
                ...
                {(() => {
                  ...
                  if (
                    ...
                    for (let i = 0; i < subjectIndex; i++) {
                      const s = config[i];
                      if (s.segments) {
                        subjectStart += s.segments.reduce(
                          (acc, seg) => acc + seg.count * seg.lengthPerItem,
                          0,
                        );
                      } else {
                    ...
                    for (let i = 0; i < seg.count; i++) {
                      ...
                      const itemGlobalStart =
                        segmentGlobalStart + i * seg.lengthPerItem;
                      const itemVal = valFull
                        .slice(
                          itemGlobalStart,
                          itemGlobalStart + seg.lengthPerItem,
                        )
                        .trim();
                      const weight =
                        currentWeights[globalQIndex] !== undefined
                          ? currentWeights[globalQIndex]
                          : seg.points;

                      renderList.push(
                        <div
                          key={i}
                          className="flex items-center gap-2 mb-2 p-2 bg-white border border-slate-100 rounded-lg shadow-sm"
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const newVal = e.target.value
                                .padEnd(seg.lengthPerItem, " ")
                                .slice(0, seg.lengthPerItem);
                              const currentFull = valFull.padEnd(
                                segmentGlobalStart +
                                  seg.count * seg.lengthPerItem,
                                " ",
                              );
                              const pre = currentFull.slice(0, itemGlobalStart);
                              const post = currentFull.slice(
                                itemGlobalStart + seg.lengthPerItem,
                              );
                          ...
                          {openWeights && onOpenWeightsChange && (
                            <div className="flex items-center gap-1 relative group">
                              <span className="text-[9px] text-slate-300 absolute -top-2 left-0 w-full text-center">
                                Bal
                              </span>
                              <input
                                type="number"
                    ...
                    return (
                      <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-1 w-full mr-8">
                          ...
                          <div className="flex items-center text-[10px] text-slate-500 gap-3">
                            <span>
                              Qapalı: <b>{fixedScore}</b>
                            </span>
                            <span>
                              Açıq Limit: <b>{totalTarget - fixedScore}</b>
                            </span>
                            <span
                              className={
                                isOver
                    ...
                    return Array.from({ length: seg.count }).map((_, qIdx) => {
                      ...
                      return (
                        <div key={qIdx} className="flex items-center gap-4">
                          <span className="w-16 text-xs font-bold text-slate-500 uppercase">
                            Sual {qIdx + 1}
                          </span>
                          <input
                            type="number"
                            ...
                            onChange={(e) => {
                              ...
                              newWeights[globalQIndex] = isNaN(numVal)
                                ? 0
                                : numVal;
                              onOpenWeightsChange(subject.id, newWeights);
                            }}
                  ...
                  const segmentStart = subject
                    .segments!.slice(0, segmentIndex)
                    .reduce((acc, s) => acc + s.count * s.lengthPerItem, 0);
                  const globalStart = start + segmentStart;
                  const segmentValue = valFull
                    .slice(
                      globalStart,
                      globalStart + seg.count * seg.lengthPerItem,
                    )
                    .padEnd(seg.count * seg.lengthPerItem, " ");

                  return Array.from({ length: seg.count }).map((_, qIdx) => {
                    const qStart = qIdx * seg.lengthPerItem;
                    const qVal = segmentValue.slice(
                      qStart,
                      qStart + seg.lengthPerItem,
                    );
...
const TableRow = React.memo(
  ({
    ...
    const maxScore = config.reduce((acc, curr) => {
      if (curr.segments)
        return (
          acc + curr.segments.reduce((sAcc, s) => sAcc + s.count * s.points, 0)
        );
                  ...
                  <div className="flex flex-wrap gap-1 mt-1 select-none">
                    {(() => {
                      ...
                      return segments.map((seg, segIdx) => {
                        const segLen = seg.count * seg.lengthPerItem;
                        const sSegment = score.studentAnswerString.slice(
                          currentIndex,
                          currentIndex + segLen,
                        );
                        const kSegment = score.correctAnswerString.slice(
                          currentIndex,
                          currentIndex + segLen,
                        );
                        currentIndex += segLen;
                        ...
                        if (seg.type === "closed") {
                          return sSegment.split("").map((char, i) => {
                            ...
                            return (
                              <span
                                key={`c-${segIdx}-${i}`}
                                className={cn(
                                  "w-4 h-4 text-[10px] flex items-center justify-center rounded border",
                                  color,
                                )}
                              >
                                {char}
                          ...
                          for (let i = 0; i < seg.count; i++) {
                            ...
                            const sVal = sSegment
                              .slice(start, start + seg.lengthPerItem)
                              .trim();
                            const kVal = kSegment
                              .slice(start, start + seg.lengthPerItem)
                              .trim();
                            ...
                            answers.push(
                              <span
                                key={`n-${segIdx}-${i}`}
                                className={cn(
                                  "px-1.5 py-0.5 text-[9px] font-mono rounded border whitespace-nowrap",
                                  color,
                                )}
                                title={`Correct: ${kVal}`}
                              >
                          ...
                          for (let i = 0; i < seg.count; i++) {
                            ...
                            answers.push(
                              <span
                                key={`o-${segIdx}-${i}`}
                                className={cn(
                                  "w-4 h-4 text-[10px] flex items-center justify-center rounded border font-bold",
                                  color,
                                )}
                                title={`Score: ${sVal}`}
                              >
                          ...
                          return (
                            <div
                              key={`g-${segIdx}`}
                              className="flex gap-0.5 ml-1 pl-1 border-l border-slate-200"
                            >
                              {answers}
                            </div>
                          );
...
export default function OMRDashboard() {
  ...
  const [examType, setExamType] = useState<"standard" | "buraxilis">(
    "standard",
  );
  const [headerConfig, setHeaderConfig] = useState<HeaderField[]>(
    DEFAULT_LEGACY_HEADER,
  );
  ...
  useEffect(() => {
    if (examType === "buraxilis") {
      ...
      setActiveClass("09");
      setHeaderConfig(DEFAULT_BURAXILIS_HEADER);
    } else {
      setConfigMap({
        default: DEFAULT_SUBJECT_CONFIG,
        ...CLASS_CONFIGS,
      ...
      setActiveClass("01");
      setHeaderConfig(DEFAULT_LEGACY_HEADER);
    }
  ...
  const [openWeights, setOpenWeights] = useState<
    Record<string, Record<string, number[]>>
  >({}); // {class: {subjectId: [w1, w2]}}
  ...
  const handleParseComplete = (results: ParsedStudent[]) => {
    ...
    results.forEach((student) => {
      if (student.isValid) {
        ...
        if (!detectedLangsBuild[student.sinif])
          detectedLangsBuild[student.sinif] = {};
        if (!detectedLangsBuild[student.sinif][student.variant])
          detectedLangsBuild[student.sinif][student.variant] = new Set();
        ...
        if (
          ...
          detectedLangsBuild[student.sinif][student.variant].add(
            student.dil.toUpperCase(),
          );
        }
      ...
      const timer = setTimeout(() => {
        try {
          const gradedResults = parsedData.map((student) => {
            ...
            const classWeights = openWeights
  ...
  }, [parsedData, answerKeys, configMap, openWeights, headerConfig]);
  ...
  const allNeededKeysFilled = useMemo(() => {
    ...
    return Object.entries(neededKeys).some(([cls, variants]) => {
      return variants.some((variant) => {
        ...
        if (langs.length > 0) {
          return langs.some(
            (l) => (answerKeys[cls]?.[`${variant}-${l}`] || "").length > 0,
          );
        }
        return false;
      });
  ...
  const handleUpdateSubject = useCallback(
    (index: number, field: keyof SubjectConfig, value: string | number) => {
      setConfigMap((prev) => {
        ...
        if (field === "name") {
          newSubject.id = (value as string).toLowerCase().replace(/\s+/g, "");
        }
        ...
        if (newTotalLength !== oldLength) {
          ...
          if (classKeys) {
            ...
            Object.keys(newKeys).forEach((variant) => {
              ...
              const subjectPart = key.slice(
                subjectStart,
                subjectStart + oldLength,
              );
  ...
  const activeConfigTotalLength = activeConfig.reduce((acc, curr) => {
    if (curr.segments) {
      return acc + curr.segments.reduce((sAcc, s) => sAcc + s.count, 0);
    }
    return acc + (curr.length || 0);
  }, 0);
  const activeConfigTotalScore = activeConfig.reduce((acc, curr) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 font-sans text-slate-900">
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        {isConfigOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-5xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="flex flex-col gap-4 p-6 border-b border-slate-100 bg-white z-10">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Ümumi ayarları və siniflər üzrə fənn və sual saylarını
                      tənzimləyin
                    </p>
                  </div>
                  <button
                ...
                {/* Header Config Section (Replaces Old start index input) */}
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <div className="flex-1">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">
                      Cavabların Başlanğıc İndeksi (Avtomatik Təyin Edilir)
                    </label>
                    <p className="text-[10px] text-slate-500 mt-1">
                      * Cavabların haradan başladığını{" "}
                      <strong>Başlıq Formatı</strong> bölməsindən tənzimləyə
                      bilərsiniz. (Cari:{" "}
              ...
              <div className="flex flex-1 overflow-hidden">
                ...
                {/* Main: Subject List or Header Config for Active Tab */}
                        ...
                        <div className="space-y-3">
                          {headerConfig.map((field, index) => {
                            ...
                            return (
                              <div
                                key={index}
                                className="flex flex-col bg-slate-50 rounded-xl border border-slate-200 p-4 shadow-sm hover:border-indigo-200 transition-colors gap-3"
                              >
                                <div className="flex items-center gap-4">
                                  <div className="flex font-mono text-slate-400 text-xs w-6 justify-center bg-slate-200 rounded-full h-6 items-center shrink-0">
                                    {index + 1}
                                  </div>
                                  <div className="flex-1 flex gap-3">
                                    <input
                                      ...
                                      placeholder="Sahə adı (Məsələn: Ad)"
                                      className="font-semibold text-slate-700 flex-1 px-3 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                                    />
                                    <select
                                      value={field.id}
                                      onChange={(e) => {
                                        const newConfig = [...headerConfig];
                                        newConfig[index].id = e.target
                                          .value as any;
                                        if (e.target.value !== "skip") {
                                          ...
                                          > = {
                                            ...
                                            soyad: "Soyad",
                                            ataAdi: "Ata Adı",
                                            isNomresi: "İş Nömrəsi",
                                            mekteb: "Məktəb",
                                            sinif: "Sinif",
                                            dil: "Bölmə (XBT)",
                                            variant: "Variant",
                                            bolme: "Bölmə",
                                            sinfinAdi: "Sinfin Adı",
                                            cins: "Cins",
                                            qrup: "Qrup",
                                          };
                                          newConfig[index].name =
                                            nameMap[e.target.value] ||
                                            newConfig[index].name;
                                        }
                                        setHeaderConfig(newConfig);
                                      }}
                                      className="text-sm font-medium text-slate-600 border border-slate-200 rounded-lg px-3 py-2 bg-white outline-none focus:ring-2 focus:ring-indigo-500 min-w-[140px]"
                                    >
                                      <option value="ad">Ad</option>
                                      <option value="soyad">Soyad</option>
                                      <option value="ataAdi">Ata Adı</option>
                                      <option value="isNomresi">
                                        İş Nömrəsi
                                      </option>
                                      <option value="mekteb">Məktəb</option>
                                      <option value="sinif">Sinif</option>
                                      <option value="dil">Bölmə (XBT)</option>
                                      <option value="variant">Variant</option>
                                      <option value="bolme">Bölmə</option>
                                      <option value="sinfinAdi">
                                        Sinfin Adı
                                      </option>
                                      <option value="cins">Cins</option>
                                      <option value="qrup">Qrup</option>
                                      <option value="skip">
                                        -- Atla (Boşluq) --
                                      </option>
                                    </select>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const newConfig = [...headerConfig];
                                        newConfig[index].length =
                                          parseInt(e.target.value) || 0;
                                        setHeaderConfig(newConfig);
                                      }}
                                      className="w-20 px-3 py-2 border border-slate-200 rounded-lg text-center font-mono text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-bold"
                                    />
                                  </div>
                                  <button
                                    onClick={() => {
                                      const newConfig = headerConfig.filter(
                                        (_, i) => i !== index,
                                      );
                                      setHeaderConfig(newConfig);
                                    }}
                                ...
                                <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-500 bg-white px-3 py-1.5 rounded-lg border border-slate-200 self-start shadow-sm mt-2">
                          ...
                          <button
                            onClick={() => {
                              setHeaderConfig([
                                ...headerConfig,
                                { id: "skip", name: "Boşluq", length: 1 },
                              ]);
                            }}
                            className="w-full py-3.5 border border-dashed border-slate-300 rounded-xl flex items-center justify-center gap-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 transition-colors font-medium text-sm mt-6"
                          >
                            <Plus className="w-5 h-5" /> Yeni Sahə Əlavə Et
                          </button>
                        </div>

                        <div className="mt-8 p-4 bg-indigo-50/80 rounded-xl border border-indigo-100 flex gap-4 text-indigo-800">
                          <AlertTriangle className="w-6 h-6 shrink-0 text-indigo-600 mt-0.5" />
                          <div className="text-sm leading-relaxed space-y-1">
                            <p>
                              <span className="font-bold">Diqqət!</span> Bu
                              bölmədəki sahələrin məzmun uzunluğu cavabların
                              text faylında neçənci simvoldan başladığını təyin
                              edir.
                            </p>
                            <p className="text-indigo-600/80 text-xs">
                              Məsələn, əgər tələbənin adı faylda 10 simvol yer
                              tutursa, lakin siz burada 15 yazsanız, növbəti
                              məlumatlar (və dolayısı ilə cavablar) səhv
                              oxunacaq.
                            </p>
                          </div>
                        </div>
        ...
        {/* Parsing Debugger (Standard / Legacy) */}
        {examType === "standard" && parsedData && parsedData.length > 0 && (
          <div className="flex flex-col gap-4">
            {Array.from(new Set(parsedData.map((d) => d.sinif)))
              .sort()
              .map((cls) => {
                const student = parsedData.find((d) => d.sinif === cls);
                if (!student || !student.originalLine) return null;
                return (
                  <StandardParsingDebugger
                    key={cls}
                    classNameTitle={cls}
                    rawLine={student.originalLine}
                    headerConfig={headerConfig}
                    subjectConfigs={configMap[cls] || configMap["default"]}
                  />
                );
              })}
          </div>
            ...
            <div className="lg:col-span-1 space-y-4">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-1 flex shadow-sm">
                <button
                  ...
                  className={cn(
                    "flex-1 px-2 py-2 rounded-lg text-xs font-bold transition-all text-center",
                    examType === "standard"
              ...
              {/* Removed Data Start Index Input from here */}
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
                      {detectedClasses.map((cls) => {
                        ...
                        const isComplete = variants.every((v) => {
                          ...
                          const checkLen = (key: string) =>
                            (answerKeys[cls]?.[key] || "").length >=
                              requiredLength && requiredLength > 0;
                          if (langs.length > 0)
                            return langs.every((l) => checkLen(`${v}-${l}`));
                          return checkLen(v);
                        });
                    ...
                    {activeClass && (
                      <div className="flex flex-col gap-6">
                        {(neededKeys[activeClass] || []).map((variant) => {
                          const langs =
                            detectedLangs[activeClass]?.[variant] || [];
                          const valuesObj: Record<string, string> = {};
                          if (langs.length > 0) {
                            langs.forEach(
                              (l) =>
                                (valuesObj[l] =
                                  answerKeys[activeClass]?.[
                                    `${variant}-${l}`
                                  ] || ""),
                            );
                          } else {
                            valuesObj[""] =
                              answerKeys[activeClass]?.[variant] || "";
                          }
                ...
                <div className="p-4 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      title={
                        !examName.trim()
                          ? "Əvvəlcə imtahan adını daxil edin"
                          : ""
                      }
                    >
                      {isSavingToDB ? (
                        ...
                        >
                          ...
                          <thead className="bg-slate-100 text-slate-500 text-xs uppercase tracking-wider">
                            <tr>
                              ...
                              <th className="px-3 py-3 font-semibold text-center bg-blue-50/50 text-indigo-600">
                                Yekun
                              </th>
                              {allUniqueSubjects.map((subject) => (
                                <th
