...
import { BookOpen, Users, Award, Globe, Brain, GraduationCap, Search, CheckCircle, Target, TrendingUp, Shield, Loader2 } from 'lucide-react'
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center relative z-10">
            <div>
              ...
              <h1 className="text-[28px] sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 sm:mb-6 leading-[1.15] text-gray-dark">
                Gələcəyinizə<br className="sm:hidden" />{' '}
              ...
              <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-3 mb-5 sm:mb-8">
                {stats.map((stat, i) => (
                  <div key={i} className="bg-gray-50 rounded-xl sm:rounded-full px-3 sm:px-5 py-2 sm:py-2.5 border border-gray-200 hover:border-brand-red/30 hover:shadow-sm transition-all text-center sm:text-left whitespace-nowrap">
                    <span className="text-base sm:text-xl font-bold text-brand-red mr-1">{stat.number}</span>
                    <span className="text-[11px] sm:text-sm text-gray-600">{stat.label}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-4">
                <a href="#exam-search">
                  <Button size="lg" className="shadow-lg w-full sm:w-auto text-sm sm:text-lg px-5 sm:px-8 py-3 sm:py-4">
                    Sınaq imtahanına yazıl
                  </Button>
                </a>
                <Link href="/filiallarimiz">
                  <Button variant="outline" size="lg" className="w-full sm:w-auto text-sm sm:text-lg px-5 sm:px-8 py-3 sm:py-4">
                    Filiallarımız
                  </Button>
                </Link>
              </div>
      ...
      <Section background="gray">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
          {services.map((service, index) => (
            <Card
              key={index}
              className="p-4 sm:p-6 lg:p-8 bg-white border border-gray-100 hover:shadow-xl transition-all duration-300 sm:hover:-translate-y-1"
            >
              <div className="w-10 h-10 sm:w-14 sm:h-14 lg:w-16 lg:h-16 rounded-xl sm:rounded-2xl bg-gradient-to-br from-brand-red/10 to-brand-red/5 flex items-center justify-center mb-3 sm:mb-5">
                <div className="text-brand-red">{service.icon}</div>
              </div>
              <h3 className="text-sm sm:text-base lg:text-xl font-bold text-gray-dark mb-1 sm:mb-2 leading-snug">{service.title}</h3>
              <p className="text-xs sm:text-sm text-gray-medium leading-relaxed hidden sm:block">{service.description}</p>
            </Card>
          ))}
        </div>
      </Section>
          ...
          <div className="grid lg:grid-cols-3 gap-4 sm:gap-8">
            ...
            <div className="lg:col-span-2">
              <Card className="p-4 sm:p-6 shadow-lg">
                <form onSubmit={handleExamSearch} className="space-y-3 sm:space-y-4">
                  {/* On mobile: stack all inputs | Desktop: 2-col */}
                  <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
                    <Select
                      label="İmtahan"
                      options={[
                        { value: '', label: isFetchingExams ? 'Yüklənir...' : 'İmtahan seçin' },
                        ...examOptions.map(exam => ({ value: exam.id, label: exam.name }))
                      ]}
                      value={examForm.examId}
                      onChange={(e: any) => setExamForm({ ...examForm, examId: e.target.value })}
                    />
                    <Select
                      label="Sinif"
                  ...
                  <Button type="submit" className="w-full text-sm sm:text-base py-3 sm:py-4" size="lg" disabled={isSearching || isFetchingExams}>
                    {isSearching ? <Loader2 className="mr-2 animate-spin" size={18} /> : <Search size={18} className="mr-2" />}
      ...
      <Section background="gray">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {whyChooseUs.map((item, index) => (
            <Card key={index} className="p-4 sm:p-6 bg-white border border-gray-100 hover:border-brand-red transition-all">
              ...
              <h3 className="text-sm sm:text-lg font-bold text-gray-dark mb-1 sm:mb-2 leading-snug">{item.title}</h3>
              <p className="text-xs sm:text-sm text-gray-medium leading-relaxed">{item.description}</p>
            </Card>
          ))}
        </div>
        ...
        <div className="mt-8 sm:mt-12 bg-white rounded-2xl p-5 sm:p-8 border border-gray-200 shadow-sm">
          <h3 className="text-xl sm:text-2xl font-bold text-gray-dark mb-4 sm:mb-6 text-center">Uğur hekayələrimiz</h3>
          ...
          <div className="flex sm:grid sm:grid-cols-3 gap-4 sm:gap-8 overflow-x-auto pb-2 sm:pb-0 -mx-1 sm:mx-0 px-1 sm:px-0 no-scrollbar">
            <div className="flex-shrink-0 w-[260px] sm:w-auto text-center bg-gray-50 sm:bg-transparent rounded-xl p-4 sm:p-0">
              ...
              <p className="text-xs sm:text-sm text-gray-600 italic leading-relaxed">&quot;Azəri kurslarında aldığım biliklərlə 700 bal toplayıb ADNSU-ya qəbul oldum&quot;</p>
              <p className="text-[11px] sm:text-xs text-gray-500 mt-2 font-semibold">- Nigar Əliyeva</p>
            </div>
            <div className="flex-shrink-0 w-[260px] sm:w-auto text-center bg-gray-50 sm:bg-transparent rounded-xl p-4 sm:p-0">
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-blue-100 flex items-center justify-center mx-auto mb-3 sm:mb-4">
                <GraduationCap size={24} className="text-blue-600 sm:w-8 sm:h-8" />
              </div>
              <p className="text-xs sm:text-sm text-gray-600 italic leading-relaxed">&quot;Müəllimlər çox peşəkar, hər dərsdə yeni biliklər öyrənirəm&quot;</p>
              <p className="text-[11px] sm:text-xs text-gray-500 mt-2 font-semibold">- Rəşad Məmmədov</p>
            </div>
            <div className="flex-shrink-0 w-[260px] sm:w-auto text-center bg-gray-50 sm:bg-transparent rounded-xl p-4 sm:p-0">
              ...
              <p className="text-xs sm:text-sm text-gray-600 italic leading-relaxed">&quot;3 ay ərzində balımı 200 xal artırdım, təşəkkürlər!&quot;</p>
              <p className="text-[11px] sm:text-xs text-gray-500 mt-2 font-semibold">- Leyla Həsənova</p>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section>
        <div className="text-center max-w-3xl mx-auto px-2">
          ...
          <div className="flex gap-3 sm:gap-4 justify-center">
            <Link href="/elaqe">
              <Button size="lg" className="text-sm sm:text-lg px-5 sm:px-8 py-3 sm:py-4">Əlaqə</Button>
            </Link>
            <Link href="/haqqimizda">
              <Button variant="outline" size="lg" className="text-sm sm:text-lg px-5 sm:px-8 py-3 sm:py-4">Haqqımızda</Button>
            </Link>
          </div>
        </div>
      </Section>
